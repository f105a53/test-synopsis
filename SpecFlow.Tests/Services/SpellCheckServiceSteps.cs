...
{
    [Binding]
    public class SpellCheckServiceSteps
    {
        private string searchTerm;

        private SpellCheckService.Core.Services.SpellCheckService service;
        private SpellCheckService.Core.Entities.Spellings results;
        ...
        public async Task Setup()
        {
            ...
            service = new SpellCheckService.Core.Services.SpellCheckService(Path.Combine(tempPath, "lucene-index"));
        }
        ...
        {
            results = service.GetSpellings(new SpellCheckService.Core.Entities.Spellings.Request { Text = "Deal" });
        }
        
        [Then(@"the first spelling suggestion should be (.*)")]
        public void ThenTheFirstSpellingSuggestionShouldBe(string firstSuggestion)
        {
            results.Should().NotBeNull();
            results.spellings.Should().NotBeNull().And.NotBeEmpty();
            results.spellings.First().Should().Be(firstSuggestion);
        }
    }
}
