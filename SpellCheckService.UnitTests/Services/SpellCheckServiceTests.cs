...
-|";
        ...
        [Theory]
        public async void GetSpellingSuggestion_InputRange(string input, bool shouldSucceed)
        {
            #region TestData
...
-|";
            ...
            var results = service.GetSpellings(new Core.Entities.Spellings.Request { Text = input });

            if (shouldSucceed)
            {
                results.Should().NotBeNull();
                results.spellings.Should().NotBeNull().And.NotBeEmpty();
                //results.spellings.First().Should().Be(input);
            }
            else
                results.spellings.Should().BeNullOrEmpty();
